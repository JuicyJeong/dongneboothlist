...
#크롬드라이버로 원하는 url로 접속
event_list = ["dice01","df2204","game03","vidol04"] ### 현재 진행중인 행사내용들을 자동으로 뽑아낼 수 있는지는 모르겠다...

for currunt_event in event_list:
    url = 'https://dongne.co/event/'+str(currunt_event)+'/circles?page=1&per_page=1000'
    driver = webdriver.Chrome('/Users/jinwoojeong/Desktop/Study/DF/chromedriver') ##크롬드라이버 주소를 입력해서 실행해야합니다.
    driver.get(url)
    time.sleep(5)

    # driver.find_element_by_xpath('//*[@id="app"]/div/main/div/div/div/section/section/div/div/div[2]/a/div[2]').click()
